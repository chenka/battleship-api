...
module.exports = battleship =  {
  ...
  deploy(gameId, options, callback) {
    Game.findOne({ gameId }, (error, game) => {
      ...
      if (!game.defender.ships[options.ship]) {
        return callback(new Error(`${options.ship} is not exist`))
      }
      ...
      if (allDeploy) {
        return callback(new Error('all ships has been deployed'))
      }
      ...
      if (game.occupyGrids.includes(gridsNumber)
        ...
        return callback(new Error(`column ${column} and row ${row} were occupied or adjacent`))
      }

      const ship = game.defender.ships[options.ship]
      if (ship.amount < 1) {
        return callback(new Error(`no more ${ship.name} ship`))
      }
      ...
      if (options.isVertical) {
        if (lastGrid > columnMaxGrid) {
          return callback(new Error('exceed vertical grid'))
        }
      } else {
        if (lastGrid > rowMaxGrid) {
          return callback(new Error('exceed horizontal grid'))
        }
      }
      let placementGrid = {
        ship: ship.name,
